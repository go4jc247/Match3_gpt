...
export class UIManager {
  ...
  _injectBaseStyles() {
    ...
    style.textContent = BASE_STYLES + EXTRA_PANEL_STYLES;
  ...
  updateGoalProgress(goals) {
    ...
    if (Array.isArray(goals)) {
      ...
      if ('current' in goals && 'target' in goals) {
        goalList = [goals];
      } else {
        goalList = Object.entries(goals).map(([key, goal]) => ({
          key,
          icon: goal.icon || '🎯',
          current: goal.current || 0,
          target: goal.target || goal.count || 0,
        }));
      }
    }
    ...
    goalList.forEach(goal => {
      ...
      if (goal.key) item.dataset.goalKey = goal.key;
      item.innerHTML = `
        <span class="lm-hud__goal-icon">${goal.icon || '🎯'}</span>
        <span class="lm-hud__goal-count">${goal.current || 0}/${goal.target || 0}</span>
      `;
      if ((goal.current || 0) >= (goal.target || 0) && (goal.target || 0) > 0) {
        item.classList.add('lm-hud__goal-item--done');
      }
  ...
  get _screenBuilders() {
    return {
      ...
      bible: (data) => this._buildBible(data),
      quizList: (data) => this._buildQuizList(data),
  ...
  _buildHUD(data = {}) {
    ...
    (data.goals || []).forEach(goal => {
      const item = this._el('div', 'lm-hud__goal-item');
      if (goal.key) item.dataset.goalKey = goal.key;
      item.innerHTML = `<span class="lm-hud__goal-icon">${goal.icon || '🎯'}</span><span class="lm-hud__goal-count">${goal.current || 0}/${goal.target || 0}</span>`;
  ...
  _buildBible(data = {}) {
    const screen = this._el('div', 'lm-panel-screen');
    const card = this._el('div', 'lm-panel-card');
    const title = this._el('h2', 'lm-panel-title', { text: 'Daily Bible Reading' });
    const ref = this._el('div', 'lm-panel-subtitle', { text: data.verse?.reference || 'Verse of the day' });
    const verse = this._el('div', 'lm-panel-body', { text: data.verse?.text || 'Take a quiet moment together today.' });
    const streak = this._el('div', 'lm-panel-meta', { text: `Streak: ${data.streak || 0} day(s)` });
    const btn = this._el('button', 'lm-btn lm-btn--primary', { text: 'Back to Menu' });
    btn.onclick = () => this._emit('bible:back');
    this._append(card, title, ref, verse, streak, btn);
    this._append(screen, card);
    return screen;
  }
  ...
  _buildQuizList(data = {}) {
    const screen = this._el('div', 'lm-panel-screen');
    const card = this._el('div', 'lm-panel-card');
    const title = this._el('h2', 'lm-panel-title', { text: 'Marriage Quizzes' });
    const intro = this._el('p', 'lm-panel-body', { text: 'Quiz content is still being built. For now, the game mode is fully playable while this section stays in preview.' });
    const list = this._el('div', 'lm-quiz-preview-list');
    (data.quizzes || []).slice(0, 5).forEach((quiz, i) => {
      const item = this._el('div', 'lm-quiz-preview-item', { text: typeof quiz === 'string' ? quiz : (quiz.title || quiz.name || `Quiz ${i + 1}`) });
      list.appendChild(item);
    });
    const btn = this._el('button', 'lm-btn lm-btn--primary', { text: 'Back to Menu' });
    btn.onclick = () => this._emit('quiz:back');
    this._append(card, title, intro, list, btn);
    this._append(screen, card);
    return screen;
  }
...
.lm-quiz-preview-list {
  display: grid;
  gap: 10px;
  margin: 10px 0 18px;
}
.lm-quiz-preview-item {
  padding: 12px 14px;
  border-radius: 14px;
