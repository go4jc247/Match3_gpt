...
class App {
  ...
  showBibleReading() {
    ...
    // Mark as read and award a small safe bonus
  ...
  startLevel(levelId) {
    ...
    this.ui.showScreen('hud', {
      ...
      starThresholds: levelConfig.starThresholds || [levelConfig.moves * 100, levelConfig.moves * 150, levelConfig.moves * 220],
      goals: (levelConfig.goals || []).map(goal => ({
        key: `${goal.type}_${goal.tile || 'any'}`,
        icon: this._goalIcon(goal),
        current: 0,
        target: goal.count || 0,
      })),
    });
  ...
  _bindEngineEvents() {
    ...
    this.engine.on('goalProgress', () => {
      const progress = this.engine.checkGoals().goals;
      const goals = Object.entries(progress).map(([key, goal]) => ({
        key,
        icon: this._goalIcon(goal),
        current: goal.current || 0,
        target: goal.count || goal.target || 0,
      }));
      this.ui.updateGoalProgress(goals);
    });
  ...
  _onTileClick(row, col) {
    ...
    this._resetHintTimer();
    if (this.renderer) this.renderer.hintTiles = [];
  ...
  async _processAnimationSequence(animSequence) {
    ...
    for (const step of animSequence) {
      switch (step.type) {
        ...
        case 'match': {
          const sourceTiles = step.tiles
            || step.removals
            || (step.matches ? step.matches.flatMap(m => m.cells) : [])
            || [];
          if (this.renderer && sourceTiles.length) {
            const matchTiles = sourceTiles.map(t =>
              Array.isArray(t) ? { row: t[0], col: t[1] } : t
            );
            await this.renderer.animateMatch(matchTiles);
          }
          if (step.score > 0 && this.renderer) {
            const first = sourceTiles[0];
            const popupRow = Array.isArray(first) ? first[0] : (first?.row ?? Math.floor(GRID_SIZE / 2));
            const popupCol = Array.isArray(first) ? first[1] : (first?.col ?? Math.floor(GRID_SIZE / 2));
            this.renderer.showScorePopup(step.score, popupRow, popupCol);
          }
          break;
        }
        ...
        case 'fall':
        case 'cascade': {
          let movements = step.movements || step.moves || [];
          ...
          if (this.renderer && movements.length) {
        ...
        case 'bomb':
          if (this.renderer) {
            await this.renderer.animatePowerupActivation(step.type === 'rocket_h' ? 'rocket_h' : step.type === 'rocket_v' ? 'rocket_v' : 'bomb', step.row ?? 0, step.col ?? 0);
            if (step.cleared?.length) {
              await this.renderer.animateMatch(step.cleared.map(c => ({ row: c[0], col: c[1] })));
            }
          }
          break;
  ...
  _goalIcon(goal) {
    const tile = goal?.tile || goal?.type || 'target';
    const iconMap = {
      kitten: '🐱',
      heart: '❤️',
      rose: '🌹',
  ...
  usePowerup(powerupType) {
    ...
    const inventory = this.powerupManager.getInventory(this.state.profile.theme || 'wife');
    const invEntry = inventory[powerupType];
    const invCount = typeof invEntry === 'object' ? (invEntry.count || 0) : (invEntry || 0);
    if (invCount <= 0) {
  ...
  _showHint() {
    ...
    if (hint && this.renderer) {
      const from = { row: hint.from[0], col: hint.from[1] };
      const to = { row: hint.to[0], col: hint.to[1] };
      this.renderer.hintTiles = [from, to];
      this.renderer.lastInteraction = 0;
      this.renderer.animateSwap(from, to).then(() => {
        if (this.renderer && this.engine) {
          this.renderer.animateSwap(to, from).then(() => {
            if (this.renderer && this.engine) {
              this._syncRenderer();
            }
          });
